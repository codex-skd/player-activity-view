...
public class ServerSyncedConfig {
    ...
    /**
     * Safe config accessors: {@link ModConfigSpec.ConfigValue#get()} throws when the spec is not
     * loaded yet (this is a SERVER config, so it is unloaded on the title screen and until a world
     * is joined). The first shader reload at startup reaches {@code ScreenParticleRenderer} before
     * configs load, so the render path must go through these accessors, which fall back to the
     * declared default until {@link #isLoaded()} turns true.
     */
