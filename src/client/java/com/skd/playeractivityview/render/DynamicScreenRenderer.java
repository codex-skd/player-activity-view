...
public class DynamicScreenRenderer {
    ...
    public DynamicScreenRenderer() {
        LevelRenderEvents.COLLECT_SUBMITS.register(this::onCollectSubmits);
    }

    public void onCollectSubmits(LevelRenderContext event) {
        ...
        for (Entry<UUID, PlayerStatus> entry : PlayerActivityClient.getPlayerStatusManagerClient().lookupPlayerToStatus.entrySet()) {
            ...
            Vec3 pos = PlayerActivityClient.getPlayerStatusManagerClient().getParticlePosition(player);
            ...
            float hw = halfW;
            float hh = halfH;
            event.submitNodeCollector().submitCustomGeometry(poseStack, renderType, (pose, buffer) -> {
    ...
    private boolean isVisible(PlayerStatusClient ps) {
