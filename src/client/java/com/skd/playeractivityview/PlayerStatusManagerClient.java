...
public class PlayerStatusManagerClient extends PlayerStatusManager {
    ...
    private final PlayerStatusClient selfPlayerStatus = new PlayerStatusClient(PlayerStatus.PlayerGuiState.NONE, null);
    private final PlayerStatusClient selfPlayerStatusPrev = new PlayerStatusClient(PlayerStatus.PlayerGuiState.NONE, null);
    ...
    public void tickGame() {
        ...
        while (it.hasNext()) {
            ...
            PlayerStatusClient ps = (PlayerStatusClient) entry.getValue();
        ...
        if (lastLevel != level) {
            for (Entry<UUID, PlayerStatus> e : lookupPlayerToStatus.entrySet()) ((PlayerStatusClient)e.getValue()).resetParticles();
    ...
    private void tickOtherPlayerClient(Player player) {
        PlayerStatusClient ps = (PlayerStatusClient) getStatus(player);
    ...
    @Override
    protected PlayerStatusClient createStatus(UUID uuid) {
        return new PlayerStatusClient(PlayerStatus.PlayerGuiState.NONE, uuid);
    }
    ...
    public void sendScreenRenderData(PlayerStatusClient status) {
    ...
    public static void receiveServerPacket(CompoundTag nbt) {
        try {
            if (nbt.contains(PlayerActivityNetworking.NBTDataPlayerUUID)) {
                ...
                PlayerActivityClient.getPlayerStatusManagerClient().receiveAny(uuid, nbt);
            } else if (nbt.contains(PlayerActivityNetworking.NBTDataServerConfig)) {
                PlayerActivityClient.getPlayerStatusManagerClient().receiveServerConfig(nbt);
            } else if (nbt.contains(PlayerActivityNetworking.NBTDataItemTransferItemStack)) {
                PlayerActivityClient.getPlayerStatusManagerClient().receiveItemMove(nbt);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void receiveAny(UUID uuid, CompoundTag data) {
        PlayerStatusClient status = (PlayerStatusClient) getStatus(uuid);
